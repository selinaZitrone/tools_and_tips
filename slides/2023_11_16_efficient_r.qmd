...
**Principles** and **tools** on making R programming more effiicent for :computer:
...
## Let R do as little as possible
...
- Use the right packages
  - [`tidyverse` packages](https://www.tidyverse.org/) can be quite fast (e.g. `readr`, `dplyr`)
  - [`data.table` package](https://rdatatable.gitlab.io/data.table/) is fast and memory efficienct
...
- `dplyr` has a C++ backend and is often faster than base R
- `data.table` is fast and memory efficiency
  - Syntax is quite different from base R and `tidyverse`
- `collapse` is a C++ based package that is specifically developed for fast data analysis
