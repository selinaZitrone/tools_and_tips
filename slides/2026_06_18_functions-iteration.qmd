...
#| label: map_example
#| eval: true
# The map function always returns it's result as a list
library(purrr)
map(1:3, sqrt)
```
...
```{r}
#| eval: true
# draw 1 random number from a normal distribution with mean 1, 2 and 3
rnorm(n = 1, mean = 1)
rnorm(n = 1, mean = 2)
rnorm(n = 1, mean = 3)
```

Just using `map` with `rnorm` does not work

```{r}
# iterator is passed to the n argument, not the mean argument
map(1:3, rnorm)
```
...
```{r}
map(1:3, \(x) rnorm(n = 1, mean = x))
# same as
map(1:3, function(x) rnorm(n = 1, mean = x))
```
