...
- **Who** is your **audience**?<br> Familiarity with the topic
- **What** are **common practices** in your field?<br>Established plot types, colors, ...
- **Where** do you present your figure?<br> Different contexts require different designs

## Contexts in science

| Context |  Things to consider                                                                                                                                            | 
| ------- |  ---------------------------------------------------------------------------------------------------------------------------------------------------------- |  
| Paper   |  - Journal requirements<br>- Usually read on PC but also print (B/W)<br> - More time → Higher complexity                                                                              |  
| Poster  |  - More open design choice<br>- Attract people from far<br>- You quickly loose people to other posters <br> - Medium complexity (depending on the event)                                                         |  
...
catC <- data.frame(
  category = "Group C",
  value = c(
    rnorm(n_C / 2, mean = 40, sd = 4),
    rnorm(n_C / 2, mean = 60, sd = 4)
  )
)
...
raincloud_plot <- ggplot(
  ...
  # Add half-points (raw data) using ggdist::stat_dots on the left
  ggdist::stat_dots(
    aes(color = category),
    side = "left",
    justification = 1.1,
...
combined_plot <- bar_chart +
  ...
  theme(
    ...
    panel.grid.major.y = element_line(color = "grey99"),
    panel.grid.minor.y = element_line(color = "grey99"),
    panel.grid.major.x = element_blank(),
    axis.text.x = element_text(size = 12),
...
lollipop_chart <- ggplot(
  ...
  coord_flip() +
  scale_y_continuous(expand = c(0.011, 0)) +
  theme_minimal() +
  theme(
    ...
    panel.grid.minor.x = element_line(color = "grey99"),
...
gdp <- gapminder |>
  ...
  annotate(
    ...
    hjust = 1.1,
    label = paste(
      c("Mean: ", format(round(mean_gdpPercap, 0), big.mark = ","), "$"),
      collapse = ""
    ),
    size = 4
  ) +
...
barplot_no_highlights <- gapminder_china_india |>
  ...
  annotate(
    ...
    y = avg_gdp + 2000,
    label = paste(
      c("Mean: ", format(avg_gdp, big.mark = ","), "$"),
      collapse = ""
    ),
    size = 4
  ) +
...
barplot_highlights_2 <- gapminder_china_india |>
  ...
  labs(
    title = sprintf(
      ...
      highlight_colors["China"],
      highlight_colors["India"]
    ),
    y = "GDP per Capita ($)",
  ) +
  annotate(
...
order_legend <- gapminder |>
  ...
  scale_y_continuous(labels = scales::label_comma(), expand = c(0.01, 0)) +
...
- Remove redundant figure elements<br>Excessive grid lines, boxes, duplicate text ...
