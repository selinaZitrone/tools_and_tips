...
barplot_highlights_1 <- gapminder_china_india |>
  ...
  geom_bar(stat = "identity", show.legend = FALSE) +
  scale_fill_manual(values = c("China" = okabe_ito[1], "India" = okabe_ito[1], "Rest of Asia" = "grey60")) +
  theme_minimal() +
  labs(
    y = "GDP per Capita ($)",
    title = "China and India still below average GDP"
...
barplot_highlights_2 <- gapminder_china_india |>
  ...
  ggplot(aes(x = reorder(country, gdpPercap), y = gdpPercap, fill = highlight)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  scale_fill_manual(values = c("China" = okabe_ito[1], "India" = okabe_ito[2], "Rest of Asia" = "grey60")) +
  theme_minimal() +
  labs(
    title = sprintf(
