...
- **Maintainability** :wrench: 
  - Can I understand, update and fix my own code?
...
- Most analyses are run many times before they are done
- We want to publish our code for others

## Today

- Basic tips for better workflows and project organization
- Aim: Increase reproducibility and maintainability
...
|   |- 01_clean_data.R
|   |- 02_PCA.R
|- functions## Create an R Studio Project
...
# Purpose: Create Figure 2 showing the relationship between body mass and bill length
...
- Write multiple files that can be called sequentially, e.g. 
  - `01_prepare-data.R`: Read raw data and prepare it for analysis
  - `02_run_models.R`: Run statistical analysis
  - `03_make_figures.R`: Create manuscript figures
- Use `source()` to source R scripts in other scripts
  - E.g. use `source("01_prepare-data.R")` in `02_run_models.R`
...
source("03_make_figures.R")
```
## Use functions

- DRY principle: Don't Repeat Yourself
  - If you notice that you copy-paste code - write a function instead
- Use `source()` to source functions in other scripts
...
Same data preparation for multiple data sets
