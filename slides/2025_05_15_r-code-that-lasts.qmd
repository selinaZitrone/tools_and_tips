...
Messy projects and workflows creep in and make things difficult!
...
While writing code, imagine how someone else (or you from two years in the future) will see the project for the first time. 
...
Test this and **send your project to a colleague** and asking them understand and run the code your analysis.

:::


# :file_folder: Organizing projects for reproducibility
...
# :page_facing_up: Let's start coding
...
Or directly use the full path to the file:

```r
temperature <- read_csv("C:/Users/Selina/path/that/only/I/have/data/temperature.csv")
```
...
Avoid `setwd()` and absolute paths!

## Use save paths

Option 1: Use RStudio projects: Project root is automatically the working directory
...
- Uses a reasonable heuristics to find your project's files
- Looks for `*.Rproj`, `.here`, `.git`, ...
- Build a path using the `here` function

```{r}
# use here::here or library(here)
read_csv(here::here("data", "temperature.csv"))
# or
read_csv(here::here("data/temperature.csv"))
```
...
```{r}
#| eval: true
multiply_by_two <- function(input) {
...
- Turn on "Format at save" and let the IDE do the job

- R Studio: Open command palette (`Ctrl/Cmd + Shift + P`), search for "format" -> "Reformat documents on save"

![](images/2025_05_15_r-code-that-lasts/RStudio_format.png){width="50%"}

- VS Code/Positron: Check the  "Format on save" setting
...
1. Project organization
  ...
  - Proper file names
2. Coding
  - Save paths with RStudio projects and `here`
  - Auto-formatting for readable code
  - Linting for code analysis
3. Documentation
  - Comments
  - README files
...
Have a look at the [checklist for reproducible projects]() before you publish or share your code
